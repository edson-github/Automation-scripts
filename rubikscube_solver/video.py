...
def empty_callback(x):
    """Empty function for callback when slider positions change.

    Need input x, this is the value the slider has changed to.
    """
    pass
...
def scan():
    ...
    while cameratesting:
        ...
        # cv2.rectangle(frame, (xtop_left,ytop_left),
    ...
    while not cameratesting:
        ...
        for index, (x, y) in enumerate(detector_stickers):
            ...
            if key == 32:
                preview = list(state)
                # draw the saved colors on the preview
                draw_recorded_stickers(frame, state)
                # convert the color to notation of the middle
                # sticker and label this as the face
                face = color_to_notation(state[4])
                # convert all colors to notation
                notation = [color_to_notation(color) for color in state]
                # update the face in the sides dictionary
                sides[face] = notation

        # show the new stickers
        # draw live sampling of face colors
